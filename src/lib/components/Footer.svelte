...
<Bounded as="footer" class="text-slate-600">
    <div class="container mx-auto flex mt-20 flex-col items-center justify-between gap-6 py-8 sm:flex-row">
        <div class="name flex flex-col items-center justify-center gap-x-4 gap-y-2 sm:flex-row sm:justify-self-start">
            <a
				href="/"
				class="text-xl font-extrabold tracking-tighter text-orange-300 transition-color duration-150 hover:text-emerald-300"
			>
				{settings.data.name}
			</a>
            <span class="text-4xl font-thin leading-[0] text-orange-500" >/</span>
			...
			<ul class="flex items-center gap-1">
				{#each settings.data.nav_item as { link, label }, index}
					<li>
						<PrismicLink
							field={link}
							class="block px-3 py-1 text-base font-bold text-orange-300 transition-colors duration-150 hover:text-emerald-300"
							>{label}</PrismicLink
						>
					</li>
					{#if index < settings.data.nav_item.length - 1}
						<span class="text-4xl font-thin leading-[0] text-orange-500" >
							/
						</span>
					{/if}
				{/each}
        ...
        <div class="socials inline-flex justify-center sm:justify-end">
			{#if isFilled.link(settings.data.github_link)}
				<PrismicLink
					...
					class="p-2 text-2xl text-orange-300 transform transition-all duration-150 hover:scale-125 hover:text-emerald-300"
