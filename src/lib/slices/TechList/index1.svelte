<script lang="ts">
	...
	onMount(()=>{
		...
		tl.fromTo(".tech-row", {
			x: (index) => { return index%2 === 0 ? gsap.utils.random(600, 400) : gsap.utils.random(-600,-400)}
...
<section data-slice-type={slice.slice_type} data-slice-variation={slice.variation} class="overflow-hidden" bind:this={component}>
    ...
    <!-- Render content for item -->
	 <div class="tech-row mb-8 flex items-center justify-center gap-4 text-orange-400" aria-label={tech_name || undefined}>
		{#each Array(15) as _, index }
			<span class="text-item text-8xl font-extrabold uppercase tracking-tighter" 
